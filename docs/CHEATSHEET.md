# ai-prd-workflow 调用速查表

## 入口

| 情况 | 调用 |
|---|---|
| 不确定从哪开始 | `ai-prd-workflow` |
| 想法或方案需要先被拷问、挑战或压力测试 | `grill-me` |
| 缺背景、目标、用户、场景或成功标准 | `clarify-requirement` |
| 需先定范围、顺序、字段或权限 | `review-brief` |
| 需完整 PRD | `write-prd` |
| 页面或交互影响评审 | `build-prototype` |
| 原型结构稳定后，需要评审前收口、减法、状态和可用性统一 | `review-prototype` |
| 复杂业务规则需独立评审、复用或维护 | `write-rules` |
| 分支、链路或异常路径文字说不清 | `make-flowchart` |

## 卡点

- 用户说“拷问”“挑战”“找漏洞”“压力测试”，或早期想法已经想推进：先走 `grill-me`。
- 用户直接说“写 PRD”，但范围、顺序、字段、权限或规则边界没定：先走 `review-brief`。
- 地址治理、状态检查、识别校验类需求：不因有规则就默认拆规则稿，只有规则复杂且需独立评审、复用或持续维护时才进入 `write-rules`。
- 已有独立规则稿：PRD 只保留规则摘要和链接，不再展开规则矩阵、计算明细、优先级和例外。
- 页面只是信息承载、没有状态或交互分歧：不默认出原型。

## 顺序

```text
ai-prd-workflow → 前置入口分流 → 确定正式交付物组合 → 按需进入 PRD / 原型 / 规则稿 / 流程图
```

前置入口分流：

| 要解决的问题 | 入口 |
|---|---|
| 值不值得做、哪里最脆弱 | `grill-me` |
| 背景、目标、用户、场景、成功标准缺失 | `clarify-requirement` |
| 范围、顺序、字段、权限、边界未拍板 | `review-brief` |

这是分流顺序，不是固定流水线；三个前置入口通常只选当前最需要的一个。

## 停止

- PRD 已清楚，原型无新增评审价值。
- 规则不复杂，不拆规则稿。
- 逻辑一句话能说清，不画流程图。
- 问题属于技术实现，不走 PRD 交付。

## 句式

```text
请使用 ai-prd-workflow，先判断这个需求该怎么收口：
……
```

```text
请使用 grill-me，先拷问一下这个想法是否值得继续推进：
……
```

```text
请使用 review-brief，先确认范围、判断顺序、字段层级和待拍板问题：
……
```

```text
请使用 write-rules，把这部分复杂业务规则口径整理成独立规则稿：
……
```
